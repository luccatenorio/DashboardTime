...
# Criar trigger (a cada 1 hora, começando agora)
...
# Criar tarefa
try {
    Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger -Settings $settings -Description "Sincroniza métricas do Meta para Supabase a cada 1 hora" -RunLevel Highest
